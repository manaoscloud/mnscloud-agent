...
run() {
  ...
  info "RUN: $*"
  if [[ "$*" =~ (^|[^[:alnum:]_./-])(apt-get|apt|dpkg)[[:space:]] ]]; then
    apt_prepare_for_lock
  fi
  bash -c "$*"
}
...
mnscloud.kamailio-softswitch.update = $(detect_executable_file /opt/mnscloud/mnscloud-kamailio-softswitch/scripts/update-kamailio-softswitch.sh)
mnscloud.nginx.update = $(detect_executable_file /opt/mnscloud/mnscloud-nginx/scripts/update-nginx.sh)
mnscloud.rabbitmq.update = $(detect_executable_file /opt/mnscloud/mnscloud-rabbitmq/scripts/update-rabbitmq.sh)
